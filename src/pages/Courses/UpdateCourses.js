...
const UpdateCourses = () => {
    ...
    const loadData = async () => {
        try {
            const response = await axios.get(`${auth.baseURL}/api/get-courses/${id}`)
    ...
    const handleSubmit = async (id, oldImage, e) => {
        ...
        try {
            const response = await axios.post(`${auth.baseURL}/api/update-courses/${id}`, { name, description, oldImage, image },
                {
                    headers: {
    ...
    return (
        <div>
            {renderApp && <>
                ...
                <form onSubmit={
                    (e) => {
                        handleSubmit(courseData.id, courseData.image, e)
                    }
                } action="">
                    <ToastContainer
                        position="top-center"
                    ...
                    />
                    <div class="mb-3">
                        <label htmlFor="exampleFormControlInput1" class="form-label">Course Name</label>
                        <input type="text" class="form-control" id="courseName" defaultValue={courseData.name} placeholder="Courses Name" />
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <img src={`${courseData.image}`} width={50} alt="" />
                    </div>
                    <input className='btn add-btn' type="submit" value={"Save"} />
                    <Link className='btn ms-3' to="/courses">Back</Link>
                </form>
            </>}
